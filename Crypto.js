...
// генерирует новый ключ для шифрования Вернама  
function generateKeyVernam(length)
{
	let key = ""; // генерируем новый ключ такой же длины, как и сообщение 
...
// Реализует шифр Вернама
// input - сообщение, key - ключ 
function Vernam(input, key) {
	if ((key.length) < (input.length)) { alert("Ключ короче сообщения, это небезопасно. Сгенерируйте ключ с помощью функции generateKeyVernam(length_message)"); }
 ...
 */
const convertToHex = function(str) {
  if(!str) str = "";
  var hs ='';
  var hv ='';
...
/**
 * Преобразование шестнадцатеричной строки в строку
 *
 * @return hex string e.g. "68656c6c6f20776f726c64" => "hello world"
 */
const convertFromHex = function(str){
  if(!str) str = "";
  var s = "";
...
/**
 * удаляет все строки из строки
возвращает текст без перевода строки
 *
 * @return string
 */
const stripLineFeeds = function(str){
  if(!str) str = "";
  var s = '';
  s = str.replace(/\n/g,'');
  s = s.replace(/\r/g,'');
...
/**
 * Преобразует строку в массив байтов кода
 возвращает символы шестнадцатеричной строки в их символ кода символа
 *
 * @return hex string e.g. "68656c6c6f20776f726c64" => "hello world"
 */
...
const encodeBase64 = function(str,utf8encode) {  // http://tools.ietf.org/html/rfc4648
  if(!str) str = "";
 ...
 */
const decodeBase64 = function(str,utf8decode) {
  if(!str) str = "";
  var b64 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";
